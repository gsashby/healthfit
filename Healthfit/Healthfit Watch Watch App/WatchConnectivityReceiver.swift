...
extension WatchConnectivityReceiver: WCSessionDelegate {
    ...
    func session(_ session: WCSession,
        ...
        if let data = Self.parse(cached) {
            DispatchQueue.main.async { self.workout = data }
        }
    }

    func session(_ session: WCSession,
                 didReceiveApplicationContext applicationContext: [String: Any]) {
        if let data = Self.parse(applicationContext) {
            DispatchQueue.main.async { self.workout = data }
        }
    }

    private static func parse(_ context: [String: Any]) -> WatchWorkoutData? {
        guard
